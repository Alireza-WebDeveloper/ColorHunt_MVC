...
class AllPaletteView extends SinglePalettePreView {
    ...
    return this._data
      .map((ObjectData) => {
        return `<div class="col">
         ...
         <button class="btn btn--Bookmark w-100  d-flex justify-content-start" data-code='${
           ObjectData.id
         }'>
            <svg class="svg--btnBookmark">
              <use href="${icon}#bookmark${
          ObjectData.bookmarked ? '-fill' : ''
        }"></use>
            </svg>
           </button>
           <button class='btn btn-Like w-100 d-flex justify-content-start align-items-center' data-code='${
             ObjectData.id
           }'>
           <svg class="svg--btnLike">
              <use href="${icon}#heart${
          ObjectData.activeLike ? '-fill' : ''
        }"></use>
            </svg>
  ...
  _windowLoading(handler) {
    window.addEventListener('load', function (e) {
      if (e.target.location.origin === e.target.location.href.slice(0, -1))
        handler();
    });
  }
