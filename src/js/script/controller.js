...
const controlAllPaletteCategoryByName = async function (
  categoryName,
  page = 1
) {
  try {
    ...
    if (
      model.state.allPalettes.result.length > 0 &&
      model.state.allCategories.size > 0
    ) {
...
const controlPagination = async function (goToPage) {
  try {
    AllPaletteView._renderLoading();
    await model.loadingGetAllPaletteCategoryByName_Page(
      model.state.allCategories.query,
      goToPage
    );
    SinglePaletteView._clear();
    AllPaletteView._render(model.state.allPalettes.result);
...
const initials = function () {
  ...
  createPaletteCategoryView._addHandlerDeletePalette(
    controlDeleteCreatePaletteCategory
  );
  CategoryByNamesView._addHandlerAllPaletteCategoryByName(
    controlAllPaletteCategoryByName
  );
  ...
  CategoryByNames2View._addHandlerAllPaletteCategoryByName(
    controlAllPaletteCategoryByName
  );
  //  Comments
  ...
  createPaletteCategoryView._addHandlerSinglePalette(
    controlGetSinglePaletteComments
  );
